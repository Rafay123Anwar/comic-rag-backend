...
# Define safe character limit for Mistral embeddings (approx 8192 tokens)
...
def retrieve_chunks(
    ...
    # -----------------------------
    # Input Protection (Truncation)
    # -----------------------------
    # Mistral embedding limit is 8192 tokens. 
    # We truncate at 20,000 chars to prevent 400 Bad Request API crashes.
    if len(query) > MAX_QUERY_CHARS:
        print(f"[WARNING] Query too long ({len(query)} chars). Truncating to {MAX_QUERY_CHARS} chars to avoid embedding crash.")
